...
def create_complex_pdbs(
    ...
    for filename, mutant_pdb in tqdm(
        ...
        complex_res_B = set(
            get_residues_with_full_backbone(complex_structure, "B", RESIDUES_COMPLEX)
        )
        mutant_res_A = set(
            get_residues_with_full_backbone(mutant_structure, "A", RESIDUES_MUTANT)
        )
...
def add_affinities(items: list[dict], params: dict) -> tuple[list[dict], list[dict]]:
...
def train_atomica_node(params):
    """
    Run the ATOMICA training script via subprocess.

    params: dict containing all required parameters, e.g.
        {
          "train_set": "...",
          "valid_set": "...",
          "task": "PDBBind",
          "gpus": [-1],  # for CPU training; for GPU, e.g. [0]
          "lr": 1e-4,
          "max_epoch": 3,
          "save_dir": "...",
          "num_workers": 1,
    ...
    cmd = [
        ...
        "--valid_set", params["valid_set"],
        "--task", params["task"],
        "--lr", str(params.get("lr", 1e-4)),
        "--max_epoch", str(params.get("max_epoch", 3)),
        "--save_dir", params["save_dir"],
        "--num_workers", str(params.get("num_workers", 1)),
        "--run_name", params.get("run_name", "ATOMICA-Run"),
        # Add other fixed args below:
        "--gpus", *map(str, params["gpus"]),
    ]

    # Add flags (boolean args) if True
    if params.get("use_wandb", False):
        cmd.append("--use_wandb")

    # Add other optional params as needed, e.g.:
